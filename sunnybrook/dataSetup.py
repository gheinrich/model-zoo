...
def export_all_contours(contours, img_path, img_name):
    ...
    img_dirname = os.path.join(img_name, 'images')
    if os.path.isdir(img_dirname):
        shutil.rmtree(img_dirname)
    os.makedirs(img_dirname)

    label_dirname = os.path.join(img_name, 'labels')
    if os.path.isdir(label_dirname):
        shutil.rmtree(label_dirname)
    ...
    for i in xrange(int(np.ceil(len(contours) / float(batchsz)))):
        ...
        for idx,ctr in enumerate(batch):
            try:
                ...
                print "[%s] Processing %d/%d" % (img_name, idx, len(contours))

                # rescale
                img *= 65536./img.max()

                filename = os.path.join(img_dirname, "%08d.tif" % idx)
                PIL.Image.fromarray(img.astype('uint16'), mode="I;16").save(filename, format='TIFF')

                filename = os.path.join(label_dirname, "%08d.png" % idx)
                PIL.Image.fromarray(label).save(filename, format='PNG')
